...
\subsection{Cross-Validating the Tokenizer}
...
\item Using one of the string kernels to identify the nearest neighbor to an infrequent token
\end{itemize}

\subsection{Larger Datasets}
Our training set was large, but not exhaustive. There are many other sources of User-Agent data on the web, and we believe that the size of our dataset could increase by an order of magnitude with reasonable effort.
